...
export async function getGlobalAuditStats() {
    ...
    const systemActions = await pool.query("SELECT COUNT(*) FROM admin_audit_logs WHERE bureau_id IS NULL AND created_at >= NOW() - INTERVAL '30 days'");
    return {
...
export async function getGlobalSecurityLogs() {
    ...
    const result = await pool.query(`SELECT aal.*, u.name as admin_name
     FROM admin_audit_logs aal
     JOIN "user" u ON aal.admin_id = u.id
     WHERE aal.bureau_id IS NULL 
     AND aal.action = ANY($1)
     ORDER BY aal.created_at DESC LIMIT 50`, [securityActions]);
    return result.rows;
}
