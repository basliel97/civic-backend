...
civic.get('/polls', citizenAuth(), async (c) => {
    try {
        ...
        const polls = await getPolls(c.get('user_id'), c.get('userRegion'), c.get('userGender'), c.get('userWorkType'), category);
...
civic.post('/reports', citizenAuth(), async (c) => {
    try {
        ...
        const report = await createReport(itemId, itemType, itemTitle, user_id, reason, description);
