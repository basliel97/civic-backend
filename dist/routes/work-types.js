...
workTypes.post('/work-types', adminAuth(), async (c) => {
    try {
        ...
        const result = await pool.query('INSERT INTO work_types (name, category, is_custom) VALUES ($1, $2, TRUE) RETURNING *', [name, category || 'other']);
        // Log admin action
        await logAdminAction(adminId, null, 'CREATE_WORK_TYPE', 'system_config', result.rows[0].id, null, result.rows[0], {});
...
workTypes.delete('/work-types/:id', adminAuth(), async (c) => {
    try {
        ...
        const oldWorkTypeResult = await pool.query('SELECT * FROM work_types WHERE id = $1', [id]);
        if (oldWorkTypeResult.rows.length === 0) {
            return c.json({ success: false, error: 'Work type not found' }, 404);
        }
        const oldWorkType = oldWorkTypeResult.rows[0];
        const result = await pool.query('UPDATE work_types SET name = name || \' (archived)\' WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return c.json({ success: false, error: 'Work type not found' }, 404);
        }
        // Log admin action
        await logAdminAction(adminId, null, 'ARCHIVE_WORK_TYPE', 'system_config', id, oldWorkType, result.rows[0], {});
