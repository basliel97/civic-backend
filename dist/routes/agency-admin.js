...
import { adminAuth, agencyAdminAuth } from '../middleware/auth.js';
import { getAgencyStats, getAgencyDetailedStats, getBureauServices, createBureauService, getBureauStaff, getAdminApplications, getAdminApplicationById, getApplicationsByService, getApplicationsGroupedByService, reviewApplication, cancelApplication, addApplicationComment, getApplicationComments, updateBureauService, deleteBureauService, updateApplication, bulkUpdateApplicationStatus, updateComment, deleteComment, getBureauAnnouncements, createBureauAnnouncement, updateAnnouncement, deleteAnnouncement, getCombinedAuditLogs, logAdminAction, bulkImportLicenses, onboardLicense } from '../services/agency.js';
import { getSuggestions, // Existing function for listing
...
agencyAdmin.get('/stats/detailed', async (c) => {
    try {
        const bureauId = c.get('bureauId');
        if (!bureauId)
            return c.json({ success: false, error: 'Bureau ID required' }, 400);
...
agencyAdmin.post('/applications/bulk-status', async (c) => {
    try {
       ...
       RETURNING ta.id, ta.application_status`, [status, bureauId, applicationIds]);
        ...
        await logAdminAction(adminId, bureauId, 'bulk_update_application_status', 'transport_applications', 'multiple', null, { count: result.rows.length, status }, { applicationIds, notes });
...
agencyAdmin.patch('/applications/:id', async (c) => {
    try {
        ...
        const oldApp = await pool.query(`SELECT ta.id, ta.application_status, ta.admin_notes, ta.delivery_tracking_number, ta.delivery_method, ta.assigned_admin_id 
       ...
       WHERE ta.id = $1 AND bs.bureau_id = $2`, [id, bureauId]);
        ...
        await logAdminAction(adminId, bureauId, 'update_application', 'transport_applications', id, oldApp.rows[0], updated, { fields: Object.keys(updates) });
        return c.json({ success: true, data: updated });
    }
    catch (error) {
        return c.json({ success: false, error: error.message }, 500);
...
agencyAdmin.delete('/applications/:id', async (c) => {
    try {
        ...
        // Log admin action
        await logAdminAction(adminId, bureauId, 'cancel_application', 'transport_applications', id, { application_status: oldStatus }, { application_status: 'cancelled' }, { reason });
...
agencyAdmin.post('/review/:id', async (c) => {
    try {
        ...
        await logAdminAction(adminId, bureauId, 'review_application', 'transport_applications', id, { application_status: oldStatus }, { application_status: updates.appStatus || oldStatus, notes: updates.notes }, { license_issued: !!result.license_id, delivery_status: result.delivery_status });
...
agencyAdmin.post('/:id/comments', async (c) => {
    try {
        ...
        // Log admin action
        await logAdminAction(adminId, bureauId, 'create_comment', 'application_comments', comment.id, null, { text: text.substring(0, 100), application_id: id }, { author_role: 'admin' });
...
agencyAdmin.put('/comments/:commentId', async (c) => {
    try {
        ...
        await logAdminAction(adminId, bureauId, 'update_comment', 'application_comments', commentId, { comment_text: old.rows[0].comment_text }, { comment_text: text });
        return c.json({ success: true, data: comment });
    }
    catch (error) {
        return c.json({ success: false, error: error.message }, 500);
...
agencyAdmin.get('/audit-logs', async (c) => {
    try {
        ...
        const appLogs = await pool.query(`SELECT 
         al.id,
         al.application_id,
         al.changed_by,
         al.old_status,
       ...
       LIMIT $2 OFFSET $3`, [bureauId, limit, offset]);
        ...
        const adminLogs = await pool.query(`SELECT 
         admin_audit_logs.id,
       ...
       LIMIT $2 OFFSET $3`, [bureauId, limit, offset]);
        ...
        const combined = [...appLogs.rows, ...adminLogs.rows].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
...
agencyAdmin.post('/announcements', async (c) => {
    try {
        ...
        // Log admin action
        await logAdminAction(adminId, bureauId, 'create_announcement', 'announcements', announcement.id, null, { title: data.title, content: data.content.substring(0, 100) }, { target_role: data.target_role || 'citizen' });
...
agencyAdmin.put('/announcements/:id', async (c) => {
    try {
        ...
        // Log admin action
        await logAdminAction(adminId, bureauId, 'update_announcement', 'announcements', id, null, { title: data.title, content: data.content?.substring(0, 100) }, { fields: Object.keys(data) });
        return c.json({ success: true, data: updated });
    }
    catch (error) {
        return c.json({ success: false, error: error.message }, 500);
...
agencyAdmin.delete('/announcements/:id', async (c) => {
    try {
        ...
        // Log admin action
        await logAdminAction(adminId, bureauId, 'delete_announcement', 'announcements', id, { is_active: true }, { is_active: false });
...
agencyAdmin.post('/change-password', async (c) => {
    try {
        ...
        const accountResult = await pool.query('SELECT password FROM "account" WHERE "user_id" = $1 AND "provider_id" = $2', [adminId, 'credential']);
        ...
        await pool.query('UPDATE "account" SET password = $1, updated_at = NOW() WHERE "user_id" = $2', [hashedPass, adminId]);
        return c.json({
            success: true,
...
agencyAdmin.get('/suggestions', async (c) => {
    try {
        ...
        if (!bureauId)
            return c.json({ success: false, error: "Bureau ID not found" }, 403);
...
agencyAdmin.get('/notifications', async (c) => {
    try {
       ...
       WHERE user_id = $1 
       ORDER BY created_at DESC LIMIT 50`, [adminId]);
