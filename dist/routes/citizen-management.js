 ...
 */
const citizenManagement = new Hono();
/**
...
citizenManagement.post("/citizens", adminAuth(), async (c) => {
    try {
        ...
        const existingFin = await pool.query('SELECT id FROM "user" WHERE username = $1 OR fin = $1', [fin]);
        ...
        const existingPhone = await pool.query('SELECT id FROM "user" WHERE phone_number = $1', [phone]);
...
citizenManagement.get("/citizens/:id", adminAuth(), async (c) => {
    try {
      ...
      FROM "user"
      WHERE id = $1 AND role = 'citizen'`, [id]);
        ...
        if (adminIds.length > 0) {
            const adminsResult = await pool.query(`SELECT id, name FROM "user" WHERE id = ANY($1)`, [adminIds]);
...
citizenManagement.patch("/citizens/:id/status", adminAuth(), async (c) => {
    try {
      ...
      SET 
        ...
        updated_at = NOW()
      WHERE id = $3 AND role = 'citizen' AND deleted_at IS NULL
      RETURNING id, username, name, status`, [status, adminId, id]);
        if (result.rows.length === 0) {
            return c.json({
                success: false,
                error: "Citizen not found or has been deleted"
...
citizenManagement.post("/citizens/bulk-status", adminAuth(), async (c) => {
    try {
      ...
      SET 
        ...
        updated_at = NOW()
      WHERE id = ANY($3) AND role = 'citizen' AND deleted_at IS NULL
      RETURNING id`, [status, adminId, ids]);
        return c.json({
            success: true,
...
citizenManagement.post("/citizens/:id/restore", adminAuth(), superAdminAuth(), async (c) => {
    try {
      ...
      RETURNING id, username, name`, [adminId, id]);
        if (result.rows.length === 0) {
            return c.json({
                success: false,
