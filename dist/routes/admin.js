...
adminRoutes.post("/forgot-password", adminAuth(), async (c) => {
    try {
        ...
        const result = await pool.query('SELECT id, role FROM "user" WHERE email = $1', [email]);
...
adminRoutes.post("/change-password", adminAuth(), async (c) => {
    try {
        ...
        const sessionResult = await pool.query('SELECT "user_id" FROM "session" WHERE token = $1', [token]);
        ...
        const userResult = await pool.query('SELECT "user_id", password FROM "account" WHERE "user_id" = $1 AND "provider_id" = $2', [user_id, 'credential']);
        ...
        // Update password
        await pool.query('UPDATE "account" SET password = $1 WHERE "user_id" = $2', [hashedPassword, user_id]);
        return c.json({
            success: true,
...
adminRoutes.post("/reset-citizen-password", adminAuth(), async (c) => {
    try {
        const { user_id, newPassword } = await c.req.json();
        ...
        const userCheck = await pool.query('SELECT role FROM "user" WHERE id = $1', [user_id]);
        ...
        const accountResult = await pool.query('UPDATE "account" SET password = $1 WHERE "user_id" = $2 RETURNING id', [hashedPassword, user_id]);
        if (accountResult.rowCount === 0) {
            // Create account entry if doesn't exist
            await pool.query('INSERT INTO "account" (id, "user_id", "account_id", "provider_id", password, "created_at", "updated_at") VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), NOW())', [user_id, user_id, 'credential', hashedPassword]);
        }
        return c.json({
            success: true,
