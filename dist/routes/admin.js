...
adminRoutes.post("/change-password", adminAuth(), async (c) => {
    try {
        ...
        const sessionResult = await pool.query('SELECT "userId" FROM "session" WHERE token = $1', [token]);
        ...
        const userResult = await pool.query('SELECT "userId", password FROM "account" WHERE "userId" = $1 AND "providerId" = $2', [userId, 'credential']);
        ...
        // Update password
        await pool.query('UPDATE "account" SET password = $1 WHERE "userId" = $2', [hashedPassword, userId]);
        return c.json({
            success: true,
