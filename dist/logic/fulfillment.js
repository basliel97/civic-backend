...
export const FulfillmentRegistry = {
    'driver_license_renewal': async (client, userId, appId) => {
        await client.query(`UPDATE driver_licenses SET expiry_date = expiry_date + INTERVAL '5 years', status = 'active', updated_at = NOW() WHERE user_id = $1`, [userId]);
    ...
    'driver_license_international': async (client, userId, appId) => {
        ...
        await client.query(`INSERT INTO certificates (user_id, application_id, certificate_type, certificate_number, expiry_date) VALUES ($1, $2, 'international_verification', $3, NOW() + INTERVAL '1 year')`, [userId, appId, certNo]);
    ...
    'driver_license_replacement': async (client, userId, appId) => {
        ...
        await client.query(`UPDATE driver_licenses SET license_number = $1, updated_at = NOW() WHERE user_id = $2`, [newNo, userId]);
    ...
    'driver_license_transfer': async (client, userId, appId, responses) => {
        ...
        await client.query(`UPDATE driver_licenses SET issuing_office = $1, last_transfer_date = NOW() WHERE user_id = $2`, [target, userId]);
    ...
    'specialty_training_info': async (client, userId, appId, responses) => {
        ...
        await client.query(`UPDATE driver_licenses SET categories = categories || $1::jsonb, updated_at = NOW() WHERE user_id = $2`, [JSON.stringify([trainingType]), userId]);
    ...
    'taxi_competency_cert': async (client, userId, appId) => {
        await client.query(`UPDATE driver_licenses SET categories = categories || '["Public Transport (Taxi)"]'::jsonb, updated_at = NOW() WHERE user_id = $1`, [userId]);
    ...
    'theory_test_scheduling': async (client, userId, appId, responses) => {
        ...
        await client.query(`INSERT INTO test_records (user_id, test_type, scheduled_date, status, office_location)
       VALUES ($1, $2, $3, 'scheduled', $4)`, [userId, testType, preferredDate, officeLocation]);
    ...
    'test_rescheduling': async (client, userId, appId, responses) => {
        ...
        const testCheck = await client.query(`SELECT id, test_type, scheduled_date, status FROM test_records
       WHERE id = $1 AND user_id = $2 AND status = 'scheduled' AND score IS NULL`, [testId, userId]);
        ...
        await client.query(`UPDATE test_records SET scheduled_date = $1, status = 'rescheduled', updated_at = NOW()
       WHERE id = $2`, [newDate, testId]);
        // Then immediately set back to scheduled (but with updated_at showing it was rescheduled)
        await client.query(`UPDATE test_records SET status = 'scheduled' WHERE id = $1`, [testId]);
    ...
    'lift_suspension': async (client, userId, appId) => {
        await client.query(`UPDATE driver_licenses SET status = 'active', updated_at = NOW() WHERE user_id = $1`, [userId]);
    ...
    'driver_info_request': async (client, userId, appId) => {
        ...
        await client.query(`INSERT INTO certificates (user_id, application_id, certificate_type, certificate_number) VALUES ($1, $2, 'driver_transcript', $3)`, [userId, appId, certNo]);
        await createNotification(client, userId, "Transcript Ready", "Your transcript is available.", '/application/', appId);
    }
};
async function createNotification(client, userId, title, message, screen, targetId) {
    await client.query(`INSERT INTO notifications (user_id, title, message, type, target_screen, target_id) VALUES ($1, $2, $3, 'success', $4, $5)`, [userId, title, message, screen || null, targetId || null]);
}
