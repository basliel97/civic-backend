 ...
 * ```
 */
export const citizenAuth = () => createMiddleware(async (c, next) => {
    ...
    try {
        ...
        // Get user details
        const userResult = await pool.query(`SELECT id, fin, role, status, region, work_type, gender, deleted_at 
       FROM "user" WHERE id = $1`, [session.user_id]);
