...
agencyManagement.put('/bureaus/:bureauId/superadmins/:id', adminAuth(), globalSuperAdminAuth(), async (c) => {
  try {
    ...
    const oldUser = await pool.query(
      `SELECT id, name, email, role, status, bureau_id FROM "user" WHERE id = $1 AND role = 'super_admin' AND deleted_at IS NULL`,
      [id]
    );
    ...
    // Log admin action
    await logAdminAction(adminId, bureauId, 'UPDATE_SUPER_ADMIN', 'user', id,
      oldUser.rows[0],
      result.rows[0],
      { updated_fields: Object.keys({ name, email, status }).filter(k => k && (name || email || status)) }
    );
...
agencyManagement.post('/staff', adminAuth(), superAdminAuth(), async (c) => {
  try {
    ...
    await pool.query(
      `UPDATE "user" SET role = 'admin', bureau_id = $1, status = 'active' WHERE id = $2`,
      [creatorBureauId, userResult.user.id]
    );
