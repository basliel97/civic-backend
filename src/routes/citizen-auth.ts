...
citizenAuthRoutes.post("/complete-register", async (c) => {
  try {
    ...
    if (!userResult || !userResult.user) throw new Error("Failed to create user in auth system");
...
citizenAuthRoutes.post("/login", async (c) => {
  try {
    ...
    const userCheck = await pool.query(
      'SELECT id, failed_login_attempts, locked_until, two_factor_enabled FROM "user" WHERE username = $1',
      [fin]
    );
    ...
    if (!loginResult) {
      ...
      let lockTime = null;
      if (newCount >= 5) lockTime = new Date(Date.now() + 15 * 60000);
    ...
    const fullUser = await pool.query(
      `SELECT id, email, name, role, bureau_id AS "bureauId" FROM "user" WHERE id = $1`,
      [loginResult.user.id]
    );
...
citizenAuthRoutes.post("/2fa/login-verify", async (c) => {
  try {
    ...
    const fullUser = await pool.query(
      `SELECT id, email, name, role, bureau_id AS "bureauId", two_factor_enabled AS "twoFactorEnabled" 
...
citizenAuthRoutes.get("/profile", citizenAuthMiddleware(), async (c) => {
  ...
  `SELECT id, username, email, name, role, status, fin, fan, phone_number,
 ...
 citizenAuthRoutes.post("/change-password", citizenAuthMiddleware(), async (c) => {
  try {
    ...
    await pool.query(
      ...
      [hashedNewPassword, user_id, 'credential']
    );
...
citizenAuthRoutes.post("/2fa/verify", citizenAuthMiddleware(), async (c) => {
  try {
    ...
    await pool.query(
      'UPDATE "user" SET "two_factor_enabled" = TRUE, updated_at = NOW() WHERE id = $1',
      [userId]
    );
...
citizenAuthRoutes.post("/2fa/disable", citizenAuthMiddleware(), async (c) => {
  try {
    ...
    await pool.query(
      'UPDATE "user" SET "two_factor_enabled" = FALSE, updated_at = NOW() WHERE id = $1',
      [userId]
    );
