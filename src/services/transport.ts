...
export async function submitApplication(data: TransportApplication) {
...
export async function getCitizenApplications(userId: string) {
  const result = await pool.query(
    `SELECT * FROM transport_applications WHERE user_id = $1 ORDER BY created_at DESC`,
    [userId]
  );
  return result.rows;
}
...
export async function createAgencyService(bureauId: string, data: any) {
  const result = await pool.query(
...
export async function getAgencyServices(bureauId: string) {
  const result = await pool.query(
    'SELECT * FROM bureau_services WHERE bureau_id = $1 AND is_active = TRUE',[bureauId]
  );
  return result.rows;
}

export async function getAgencyStaff(bureauId: string) {
  const result = await pool.query(
