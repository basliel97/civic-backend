...
function calculateUserEligibility(poll: any, userRegion?: string, userGender?: string, userWorkType?: string): boolean {
  ...
  if (criteria.regions && criteria.regions.length > 0) {
    ...
    const allowedRegions = criteria.regions.map((r: string) => r.toLowerCase());
  ...
  if (criteria.genders && criteria.genders.length > 0) {
    const allowedGenders = criteria.genders.map((g: string) => g.toLowerCase());
  ...
  if (criteria.work_types && criteria.work_types.length > 0) {
    const allowedWorks = criteria.work_types.map((w: string) => w.toLowerCase());
 ...
 */
export async function getPolls(user_id?: string, userRegion?: string, userGender?: string, userWorkType?: string, category?: string) {
  const now = new Date();
  ...
  const processedPolls = await Promise.all(result.rows.map(async (poll) => {
    ...
    if (hasVoted || isExpired) {
       const votesRes = await pool.query(
         `SELECT option_index, COUNT(*)::INT as count FROM poll_votes WHERE poll_id = $1 GROUP BY option_index`, [poll.id]
       );
...
export async function createPoll(data: Partial<Poll>, createdBy: string) {
  ...
  const result = await pool.query(
    `INSERT INTO polls (title, description, options, target_criteria, start_date, end_date, status, allow_view_results_before_vote, allow_view_results_after_vote, show_results_live, created_by, category)
...
export async function updatePoll(id: string, data: Partial<Poll>) {
  ...
  for (const field of fields) {
    if (data[field as keyof Poll] !== undefined) {
      let val = data[field as keyof Poll];
      if (field === 'target_criteria' && typeof val === 'object') val = JSON.stringify(val);
...
export async function votePoll(
  ...
  if (pollRes.rows.length === 0) throw { code: 'NOT_FOUND', message: 'Poll not found' };
  
  const poll = pollRes.rows[0];

  if (poll.status !== 'active') throw { code: 'INACTIVE', message: 'Poll is not currently active' };
  ...
  const existingVote = await pool.query(
    'SELECT id FROM poll_votes WHERE poll_id = $1 AND user_id = $2', [pollId, user_id]
  );
  if (existingVote.rows.length > 0) throw { code: 'ALREADY_VOTED', message: 'You have already participated in this poll.' };
 ...
 * 8. GET RESULTS
 */
export async function getPollResults(pollId: string, user_id?: string, userRole?: string) {
  const pollRes = await pool.query('SELECT * FROM polls WHERE id = $1', [pollId]);
  if (pollRes.rows.length === 0) throw { code: 'NOT_FOUND', message: 'Poll not found' };
  ...
  const votesRes = await pool.query(
    `SELECT option_index, COUNT(*)::INT as count FROM poll_votes WHERE poll_id = $1 GROUP BY option_index`, [pollId]
  );
