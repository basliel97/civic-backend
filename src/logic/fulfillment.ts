 ...
 */
export const FulfillmentRegistry: Record<string, (userId: string, appId: string, responses: any) => Promise<void>> = {
  ...
  'driver_license_international': async (userId, appId) => {
    ...
    await pool.query(
      `INSERT INTO certificates (user_id, application_id, certificate_type, certificate_number, expiry_date)
       VALUES ($1, $2, 'international_verification', $3, NOW() + INTERVAL '1 year')`,
      [userId, appId, certNo]
    );
  ...
  'test_rescheduling': async (userId, appId, responses) => {
    ...
    await pool.query(
      `INSERT INTO test_records (user_id, scheduled_date, status)
  ...
  'driver_info_request': async (userId, appId) => {
    ...
    await pool.query(
      `INSERT INTO certificates (user_id, application_id, certificate_type, certificate_number)
 ...
 */
async function createNotification(userId: string, title: string, message: string, screen?: string, targetId?: string) {
  await pool.query(
    `INSERT INTO notifications (user_id, title, message, type, target_screen, target_id) VALUES ($1, $2, $3, 'success', $4, $5)`,
    [userId, title, message, screen || null, targetId || null]
  );
}